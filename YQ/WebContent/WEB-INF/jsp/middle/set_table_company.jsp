...
<div class="company-middle">
	...
	<input name='companyName' class='message-input' type='text' placeholder="搜索/添加的企业名" value='${messageInput}'/>
	<input title='点击添加用户' class='message-search' type="button" value='搜' onclick='submit_form("/YQ/set/company/search");'/>
	<input title='点击添加用户' class='message-add' type="button" value='+' onclick='submit_form("/YQ/set/company/add")'/>
	</form>
	<div class="company">
	<table>
	...
	<tr id="message-${item.companyName }"><td class="company-name">${item.companyName }</td>
	    ...
	    <div class="message">
	    <form action="${pageContext.request.contextPath }/set/company/update" method="post">
		...
		if (confirm(msg) == true){window.open("${pageContext.request.contextPath }/set/company/delete?companyName=${item.companyName }","_self")}'><a>删除</a></td></tr>
	</c:forEach>
	</table>
	</div>
</div>